...
class AttackWPA(Attack):
    ...
    def run(self):
        '''Initiates full WPA2 handshake capture attack (works for WPA/WPA2).'''

        # Skip if target is not WPS
        if Configuration.wps_only and self.target.wps == False:
            Color.pl('\r{!} {O}Skipping WPA2-Handshake attack on {R}%s{O} because {R}--wps-only{O} is set{W}' % self.target.essid)
            self.success = False
            return self.success
        ...
        Color.pl('\n{+} {C}Cracking WPA2 Handshake:{W} Running {C}aircrack-ng{W} with' +
        ...
        if key is None:
            ...
            Color.pl('{+} {G}Cracked WPA2 Handshake{W} PSK: {G}%s{W}\n' % key)
    ...
    def capture_handshake(self):
        ...
        with Airodump(channel=self.target.channel,
            ...
            Color.pattack('WPA2', self.target, 'Handshake capture', 'Waiting for target to appear...')
            ...
            if Configuration.ignore_old_handshakes == False:
                ...
                if handshake:
                    Color.pattack('WPA2', self.target, 'Handshake capture', 'found {G}existing handshake{W} for {C}%s{W}' % handshake.essid)
            ...
            while handshake is None and not timeout_timer.ended():
                ...
                # Only re-check when capture file changes
                if stat.st_size == last_cap_size and stat.st_mtime == last_cap_mtime:
                    time.sleep(step_timer.remaining())
                    continue

                last_cap_size = stat.st_size
                last_cap_mtime = stat.st_mtime
        ...
        if handshake is None:
            ...
            Color.pl('\n{!} {O}WPA2 handshake capture {R}FAILED:{O} Timed out after %d seconds' % (Configuration.wpa_attack_timeout))
            return handshake
        else:
