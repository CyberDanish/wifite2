...
class Airmon(Dependency):
    ...
    def start(iface):
        ...
        if len(monitor_interfaces) == 0:
            ...
            raise Exception(
                ...
                'Your driver may not support monitor mode or airmon-ng failed.'
            )

        # If airmon-ng output parsing failed, try to infer the monitor interface
        if enabled_iface is None:
            if len(monitor_interfaces) == 1:
                enabled_iface = monitor_interfaces[0]
            else:
                for mon in monitor_interfaces:
                    if mon.startswith(iface_name) or iface_name in mon:
                        enabled_iface = mon
                        break
    ...
    def ask():
        ...
        try:
            ...
            if count == 1:
                raise
            retry = raw_input(Color.s('{?} Try another interface? (y/N): '))
